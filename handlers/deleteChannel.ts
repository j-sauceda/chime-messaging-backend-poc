...
export const handler: APIGatewayProxyHandler = async (event: APIGatewayEvent) => {
  ...
  try {
    ...
    logger.info('Processing request to delete chime messaging channel');

    const messagingService = new MessagingService();
    const data = await messagingService.deleteChannel(channelARN, appInstanceUserARN);
    return {
      ...response,
    ...
    logger.error('Error deleting messaging channel: ', error);
    return {
      ...response,
